...
Sentry.init({
  ...
  release: `world-monitor@${__APP_VERSION__}`,
  environment: location.hostname === 'world-monitor.app' ? 'production'
...
Object.defineProperty(window, 'beta', {
  get() {
    const on = localStorage.getItem('world-monitor-beta-mode') === 'true';
    console.log(`[Beta] ${on ? 'ON' : 'OFF'}`);
    return on;
  },
  set(v: boolean) {
    if (v) localStorage.setItem('world-monitor-beta-mode', 'true');
    else localStorage.removeItem('world-monitor-beta-mode');
    location.reload();
  },
});
